...
def read_sampled_data_for_trace(video_data_folder, video, user):
    path = os.path.join(video_data_folder, video, user)
...
def get_video_ids(video_data_folder):
    list_of_videos = [o for o in os.listdir(video_data_folder) if not o.endswith('.gitkeep')]
    # Sort to avoid randomness of keys(), to guarantee reproducibility
    list_of_videos.sort()
    return list_of_videos

# returns the unique ids of the users in the dataset
def get_user_ids(video_data_folder):
    videos = get_video_ids(video_data_folder)
    users = np.array([])
    for video in videos:
        user=np.load(os.path.join(video_data_folder,video,f'{video}_users.npy'))
        users=np.union1d(users,user)
...
def get_users_per_video(video_data_folder):
    videos = get_video_ids(video_data_folder)
    users_per_video = {}
    for video in videos:
        users_per_video[video] = np.load(os.path.join(video_data_folder,video,f'{video}_users.npy'))
...
def partition_in_train_and_test_without_any_intersection(video_data_folder, init_window, end_window, videos_train, videos_test, users_train, users_test):
    partition = {}
    partition['train'] = []
    partition['test'] = []
...
def partition_in_train_and_test_without_video_intersection(video_data_folder, init_window, end_window, videos_train, videos_test, users_per_video):
    partition = {}
    partition['train'] = []
    partition['test'] = []
    for video in videos_train:
