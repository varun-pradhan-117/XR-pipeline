...
class PositionDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        if self.model_name not in ['pos_only', 'pos_only_3d_loss', 'MM18']:
            encoder_sal_inputs_for_batch=self.all_saliencies[video][tstamp-self.M_WINDOW+1:tstamp+1]
            decoder_sal_inputs_for_batch=self.all_saliencies[video][tstamp+1:tstamp+self.future_window+1]
        if self.model_name == 'CVPR18_orig':
            encoder_pos_inputs_for_batch=self.all_traces[video][user][tstamp-self.M_WINDOW+1:tstamp+1]
            decoder_outputs_for_batch=self.all_traces[video][user][tstamp+1:tstamp+1+1]
        elif self.model_name == 'MM18':
            encoder_sal_inputs_for_batch=torch.cat((self.all_saliencies[video][tstamp-self.M_WINDOW+1:tstamp+1], self.all_headmaps[video][user][tstamp-self.M_WINDOW+1:tstamp+1]), dim=1)
            decoder_outputs_for_batch=self.all_headmaps[video][user][tstamp+self.future_window+1]
        else:
            encoder_pos_inputs_for_batch=self.all_traces[video][user][tstamp-self.M_WINDOW:tstamp]
            decoder_pos_inputs_for_batch=self.all_traces[video][user][tstamp:tstamp+1]
            decoder_outputs_for_batch=self.all_traces[video][user][tstamp+1:tstamp+self.future_window+1]
